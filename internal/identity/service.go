...
// SuspendTenantIdentities is intentionally a separate persistence operation used by the tenant workflow.
// Its caller is responsible for composing it with the tenant state and audit transaction.
func (s *Service) SuspendTenantIdentities(ctx context.Context, tenantID string) error {
	if _, err := s.store.DB().ExecContext(ctx, `UPDATE users SET active=0 WHERE tenant_id=?`, tenantID); err != nil {
		return fmt.Errorf("suspend tenant users: %w", err)
	}
	if _, err := s.store.DB().ExecContext(ctx, `UPDATE sessions SET revoked=1 WHERE user_id IN (SELECT id FROM users WHERE tenant_id=?)`, tenantID); err != nil {
