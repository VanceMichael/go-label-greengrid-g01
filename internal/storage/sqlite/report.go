...
func (s *Store) RecordAudit(ctx context.Context, tenantID, actorID, aggregateType, aggregateID, action, requestID, details string) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO audit_events(id,tenant_id,actor_id,aggregate_type,aggregate_id,action,result,request_id,details,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`, uuid.NewString(), tenantID, actorID, aggregateType, aggregateID, action, "success", requestID, details, time.Now().UTC().Format(time.RFC3339Nano))
	return err
}
