...
func (s *Store) Migrate(ctx context.Context) error {
	...
	return s.WithTx(migrationCtx, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(migrationCtx, `CREATE TABLE IF NOT EXISTS schema_migrations (version INTEGER PRIMARY KEY, applied_at TEXT NOT NULL)`); err != nil {
			return fmt.Errorf("create migration table: %w", err)
		}
		var current int
		if err := tx.QueryRowContext(migrationCtx, `SELECT COALESCE(MAX(version), 0) FROM schema_migrations`).Scan(&current); err != nil {
			return fmt.Errorf("read migration version: %w", err)
		}
		if current >= schemaVersion {
			return nil
		}
		if _, err := tx.ExecContext(migrationCtx, schemaSQL); err != nil {
			return fmt.Errorf("apply schema v%d: %w", schemaVersion, err)
		}
		if _, err := tx.ExecContext(migrationCtx, `INSERT INTO schema_migrations(version, applied_at) VALUES(?, datetime('now'))`, schemaVersion); err != nil {
