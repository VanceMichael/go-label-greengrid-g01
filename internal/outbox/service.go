...
func (s *Service) EnqueueTx(ctx context.Context, tx *sql.Tx, tenantID, kind, aggregateID, payload, now string) error {
	_, err := tx.ExecContext(ctx, `INSERT INTO outbox_events(id,tenant_id,kind,aggregate_id,payload,status,attempts,next_attempt_at,created_at) VALUES(?,?,?,?,?,'pending',0,?,?)`, uuid.NewString(), tenantID, kind, aggregateID, payload, now, now)
	return err
}
