...
func NewService(store *sqlite.Store) *Service { return &Service{store: store, outbox: outbox.NewService(store, 5)} }
...
func (s *Service) Promote(ctx context.Context, tenantID, actorID, artifactID, versionID string, expectedArtifactVersion int64, requestID string) error {
	...
	var version int64
	if err := s.store.DB().QueryRowContext(ctx, `SELECT a.active_version_id,a.status,v.status,a.version FROM artifacts a JOIN artifact_versions v ON v.id=? WHERE a.id=? AND a.tenant_id=?`, versionID, artifactID, tenantID).Scan(&current, &status, &old, &version); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return domain.ErrNotFound
			}
			return err
	...
	result, err := s.store.DB().ExecContext(ctx, `UPDATE artifacts SET active_version_id=?,status='promoted',version=version+1 WHERE id=? AND tenant_id=? AND version=? AND (active_version_id IS NULL OR active_version_id<>?)`, versionID, artifactID, tenantID, expectedArtifactVersion, versionID)
	if err != nil {
		return err
	}
	...
	return s.store.WithTx(ctx, func(tx *sql.Tx) error {
		result, err := tx.ExecContext(ctx, `UPDATE artifact_versions SET status='promoted',version=version+1 WHERE id=? AND status='scanned'`, versionID)
		if err != nil {
			return err
		}
		n, _ := result.RowsAffected()
		...
		if err := audit(tx, tenantID, actorID, "artifact", artifactID, "promote", requestID, "promoted"); err != nil {
			return err
		}
		now := time.Now().UTC().Format(time.RFC3339Nano)
		return s.outbox.EnqueueTx(ctx, tx, tenantID, "artifact.promoted", artifactID, `{"status":"promoted"}`, now)
	})
}
